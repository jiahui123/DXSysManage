...
namespace MH.Util
{
    ...
    public class Time
    {
        ...
        public static DateTime UserNow
        {
            get
            {
                string TimeZoneName = "W. Europe Standard Time";//这里待定 从缓存中获取用户配置的时区信息     这里测试的时候先使用欧洲时区

                return Time.ConvertTimeFromUtcByTimeZoneName(Now, TimeZoneName);
            }
        }
        ...
        {
            return Now.AddDays(i).ToString("yyyy-MM-dd");
        }
        /// <summary>
        ///  当前日期 加添加，减天数 -1、1
        /// </summary>
        ...
        {
            return Now.AddDays(i).ToString(dateformat);
        }
        #endregion
        ...
        #region add by hu 2013-8-12 为了ebay的数据库保存的UTC时间转化成当前时区时间而加入

        /// <summary>
        /// 将UTC时间转换成某个时区的时间（add by hu）(正确)
        /// </summary>
        /// <param name="datetime">UTC时间</param>
        /// <param name="ORG_ID">国别</param>
        /// <returns></returns>
        public static DateTime ConvertTimeFromUtc(DateTime datetime, int ORG_ID)
        {
            //return DateTime.Parse(datetime);
            string timezonename = getTimeZoneNameByORG_ID(ORG_ID);//获取到timezonename
            TimeZoneInfo zoneinfo = TimeZoneInfo.FindSystemTimeZoneById(timezonename);//根据timezonename获取到TimeZoneInfo
            return TimeZoneInfo.ConvertTimeFromUtc(datetime, zoneinfo);//返回时区的时间
        }

        /// <summary>
        /// 将UTC字符串转换成某个时区的时间（add by hu）(正确,字符串必须是UTC的写法)
        /// </summary>
        /// <param name="datetime"></param>
        /// <param name="ORG_ID">国别</param>
        /// <returns></returns>
        public static DateTime ConvertTimeFromUtc(string datetime, int ORG_ID)
        {
            //return DateTime.Parse(datetime);
            string timezonename = getTimeZoneNameByORG_ID(ORG_ID);//获取到timezonename
            TimeZoneInfo zoneinfo = TimeZoneInfo.FindSystemTimeZoneById(timezonename);//根据timezonename获取到TimeZoneInfo
            return TimeZoneInfo.ConvertTimeFromUtc(DateTime.Parse(datetime).ToDate(), zoneinfo);//返回时区的时间
        }

        /// <summary>
        /// 将本地时间转化成UTC时间（add by hu）
        /// </summary>
        /// <param name="datetime"></param>
        /// <param name="ORG_ID">国别</param>
        /// <returns></returns>
        public static DateTime ConvertTimeToUtc(DateTime datetime, int ORG_ID)
        {
            //return DateTime.Parse(datetime);
            string timezonename = getTimeZoneNameByORG_ID(ORG_ID);//获取到timezonename
            TimeZoneInfo zoneinfo = TimeZoneInfo.FindSystemTimeZoneById(timezonename);//根据timezonename获取到TimeZoneInfo
            return TimeZoneInfo.ConvertTimeToUtc(datetime, zoneinfo);//返回时区的时间
        }


        /// <summary>
        /// 将CST时间转化成UTC时间（add by hu）
        /// </summary>
        /// <param name="datetime"></param>
        /// <returns></returns>
        public static DateTime ConvertCSTTimeToUtc(DateTime datetime)
        {
            //return DateTime.Parse(datetime);
            string timezonename = "Central America Standard Time";
            TimeZoneInfo zoneinfo = TimeZoneInfo.FindSystemTimeZoneById(timezonename);//根据timezonename获取到TimeZoneInfo
            return TimeZoneInfo.ConvertTimeToUtc(datetime, zoneinfo);//返回时区的时间
        }

        /// <summary>
        /// 将EST时间转化成UTC时间（add by hu）
        /// </summary>
        /// <param name="datetime"></param>
        /// <returns></returns>
        public static DateTime ConvertESTTimeToUtc(DateTime datetime)
        {
            //return DateTime.Parse(datetime);
        ...
        /// <summary>
        /// 根据国家获取到timezonename（add by hu）
        /// </summary>
        /// <param name="ORG_ID"></param>
        /// <returns></returns>
        private static string getTimeZoneNameByORG_ID(int ORG_ID)
        {
            //return "W. Europe Standard Time";
            string timezonename = string.Empty;
            switch (ORG_ID)
            {
                case (int)AppEnum.ORG_ID.US:  //美国
                    timezonename = "Pacific Standard Time";//太平洋时区
                    break;
                case (int)AppEnum.ORG_ID.UK: //英国
                    timezonename = "GMT Standard Time";//格林威治时间
                    break;
                case (int)AppEnum.ORG_ID.DE: //德国
                    timezonename = "W. Europe Standard Time";//
                    break;
                case (int)AppEnum.ORG_ID.CN: //中国
                    timezonename = "China Standard Time";//中国标准时间
                    break;
                case (int)AppEnum.ORG_ID.CA: //加拿大
                    timezonename = "Eastern Standard Time";
                    break;
                default:
                    timezonename = "W. Europe Standard Time";//欧洲
                    break;
            }
            return timezonename;
        }
        ...
        /// </summary>
        /// <param name="datetime">UTC时间</param>
        /// <param name="TimeZoneName">时区</param>
        /// <returns></returns>
        public static DateTime ConvertTimeFromUtcByTimeZoneName(DateTime datetime, string TimeZoneName)
        {
            ////return DateTime.Parse(datetime);
            //string timezonename = getTimeZoneNameByORG_ID(ORG_ID);//获取到timezonename
            TimeZoneInfo zoneinfo = TimeZoneInfo.FindSystemTimeZoneById(TimeZoneName);//根据timezonename获取到TimeZoneInfo
        ...
        /// <param name="TimeZoneName">时区</param>
        /// <returns></returns>
        public static DateTime ConvertTimeToUtcByTimeZoneName(DateTime datetime, string TimeZoneName)
        {
            //return DateTime.Parse(datetime);
            //string timezonename = getTimeZoneNameByORG_ID(ORG_ID);//获取到timezonename
            TimeZoneInfo zoneinfo = TimeZoneInfo.FindSystemTimeZoneById(TimeZoneName);//根据timezonename获取到TimeZoneInfo
